...
final class AdminTelephonyWorkspaceServiceTest extends TestCase
{
    public function testWorkspaceCombinesTelephonySections(): void
    {
        ...
        $this->assertSame('didww', $workspace['vectavoip_inventory']['items'][0]['provider_code']);
        $this->assertSame('Main DIDWW Trunk', $workspace['vectavoip_inventory']['items'][0]['provider_trunk_name']);
        $this->assertSame('ORD-100', $workspace['vectavoip_inventory']['items'][0]['order_reference']);
    }
    ...
    private function service(): AdminTelephonyWorkspaceService
    {
        ...
        $pdo->exec('CREATE TABLE cc_vectavoip_did_inventory (id INTEGER PRIMARY KEY AUTOINCREMENT, did TEXT, country TEXT, region TEXT, monthly_rate REAL, setup_rate REAL, currency TEXT, status TEXT, provider_code TEXT, provider_reference TEXT, provider_trunk_reference TEXT, provider_trunk_name TEXT, order_reference TEXT, created_at TEXT, updated_at TEXT)');
        ...
        $pdo->exec('CREATE TABLE ps_endpoints (id TEXT PRIMARY KEY, transport TEXT, aors TEXT, auth TEXT, context TEXT, identify_by TEXT, disallow TEXT, allow TEXT, direct_media TEXT, rtp_symmetric TEXT, force_rport TEXT, rewrite_contact TEXT)');
        ...
        $pdo->exec("INSERT INTO cc_vectavoip_did_inventory (id, did, country, region, monthly_rate, setup_rate, currency, status, provider_code, provider_reference, provider_trunk_reference, provider_trunk_name, order_reference, created_at, updated_at) VALUES (1, '+15551230000', 'US', 'NY', 1.25, 0.00, 'USD', 'available', 'didww', 'didww:did-1', 'trunk-1', 'Main DIDWW Trunk', 'ORD-100', '2026-05-05', '2026-05-05')");
        $pdo->exec("INSERT INTO ps_endpoints (id, transport, aors, auth, context, identify_by, disallow, allow, direct_media, rtp_symmetric, force_rport, rewrite_contact) VALUES ('trunk-business', 'transport-udp', 'trunk-business', 'trunk-business-auth', 'from-pstn', 'username,ip', 'all', 'ulaw,alaw', 'no', 'yes', 'yes', 'yes')");
