...
final class RestApiControllerTest extends TestCase
{
    ...
    public function testListsAndCreatesTariffPlansThroughApi(): void
    {
        $pdo = $this->pdo();
        $controller = $this->controller('secret-key', $pdo);

        $list = $controller->handle('tariff-plans', new JsonRequest('GET', ['search' => 'Retail'], [], [
            'Authorization' => 'Bearer secret-key',
        ]));
        $create = $controller->handle('tariff-plans', new JsonRequest('POST', [], [
            'tariff' => [
        ...
        $this->assertSame('Retail', $list->getPayload()['data']['tariff-plans'][0]['tariffname']);
        $this->assertSame(201, $create->getStatusCode());
        $this->assertSame('Wholesale', $create->getPayload()['data']['tariff_plan']['tariffname']);
        $this->assertSame('tariff-plans.create', $pdo->query('SELECT action FROM cc_a2bp_audit_log')->fetchColumn());
    }

    public function testUpdatesTariffGroupThroughApi(): void
    {
        $controller = $this->controller('secret-key', $this->pdo());
        $response = $controller->handle('tariff-groups', new JsonRequest('PUT', [], [
            'id' => '1',
            'tariff' => [
    ...
    public function testLoadsInvoiceDetailWithDownloadMetadata(): void
    {
        ...
        $this->assertSame('12.00000', $payload['data']['tax_summary']['total']);
        $this->assertSame('2.00000', $payload['data']['tax_summary']['tax_total']);
    }
    ...
    public function testListsReceiptsThroughReceiptModuleFilters(): void
    {
        $controller = $this->controller('secret-key', $this->pdo());
        $response = $controller->handle('receipts', new JsonRequest('GET', [
            'from' => '2026-05-03',
            'to' => '2026-05-04',
            'customer_id' => '1',
            'status' => '0',
    ...
    {
        $controller = $this->controller('secret-key', $this->pdo());
        $response = $controller->handle('receipts', new JsonRequest('GET', ['status' => 'closed'], [], [
            'Authorization' => 'Bearer secret-key',
        ]));
    ...
    private function pdo(): PDO
    {
        ...
        $pdo->exec("INSERT INTO cc_invoice_item (id, id_invoice, date, price, VAT, description) VALUES (1, 1, '2026-05-03', '10.00000', '20.00', 'Calls')");
        $pdo->exec("INSERT INTO cc_receipt (id, id_card, title, date, status, description) VALUES (1, 1, 'Receipt', '2026-05-03', 0, 'Test receipt')");
