...
final class ProviderWebhookRepository
{
    ...
    public function record(
        ...
        $statement = $this->pdo->prepare(
            'INSERT INTO cc_provider_webhook_event
    ...
    public function hasEvent(string $provider, string $eventId): bool
    {
        ...
        $statement = $this->pdo->prepare(
            'SELECT COUNT(*) FROM cc_provider_webhook_event WHERE provider = ? AND event_id = ?'
        );
    ...
    private function ensureTable(): void
    {
        if ($this->pdo->getAttribute(\PDO::ATTR_DRIVER_NAME) === 'sqlite') {
            ...
            $this->pdo->exec('CREATE UNIQUE INDEX IF NOT EXISTS uniq_provider_webhook_event ON cc_provider_webhook_event (provider, event_id)');
            return;
        }
